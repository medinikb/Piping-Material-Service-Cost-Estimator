...
const bomColumnAliases = {
  ...
  rawOverride: [
    "rawsteelrskgoverride",
    "rawsteelrateoverride",
    "rawsteelpriceoverride",
    "rawsteelrskg",
    "rawsteel",
    "rawsteelrate",
    "rawsteelprice",
...
function renderLineItems() {
  ...
  if (lineItems.length === 0) {
    elements.lineItemsBody.innerHTML = `
      <tr class="empty-row">
        <td colspan="13">No pipe lines added yet.</td>
      </tr>
    `;
    ...
    .map(
      (item) => `
        <tr>
          ...
          <td data-label="Raw Steel Rs/kg"><span class="raw-steel-rate">${formatCurrency(item.rawSteel, 2)}</span>${
            item.rawSteelSource === "bomOverride" ? '<span class="fallback-pill raw-steel-override">BOM override</span>' : ""
          }</td>
...
function buildComponentCostEstimate({
  ...
  // BOM row overrides have first priority. This keeps the Component Cost Review
  // consistent with the uploaded template's documented override columns.
  const bomRawOverride = parseBomNumber(rawOverrideText);
  const bomFactorOverride = parseBomNumber(factorOverrideText);
  const hasBomRawOverride = Number.isFinite(bomRawOverride) && bomRawOverride > 0;
  const hasBomFactorOverride = Number.isFinite(bomFactorOverride) && bomFactorOverride > 0;
  const rawOverride = hasBomRawOverride
    ? bomRawOverride
    : rawMapping?.recommended || elements.rawOverride.value || rawSteelByYear[elements.year.value] || rawSteelByYear[2026];
  const factorOverride = hasBomFactorOverride ? bomFactorOverride : elements.factorOverride.value;
  ...
  const pipeBasis = buildEstimate({
    ...
    coating: elements.coating.value,
    rawOverride,
    rawSteelSource: hasBomRawOverride ? "bomOverride" : rawMapping ? "materialLibrary" : "defaultYear",
    rawBasisNote: hasBomRawOverride
      ? "BOM-entered Raw Steel Rs/kg Override"
      : rawMapping?.note || "Component estimate used current/default raw material basis",
    factorOverride,
  });
  ...
  const defaultMedianFactor =
    ...
    pipeBasis.factors.median;
  const defaultDisplayFactor =
    studBoltUnitEstimate?.commercialFactor ??
    elbowUnitEstimate?.elbowFactor ??
    equalTeeUnitEstimate?.teeFactor ??
    unequalTeeUnitEstimate?.teeFactor ??
  ...
  // Special component methods use their own commercial/P50 factor. When a BOM
  // factor override is supplied, replace that one rate-conversion factor while
  // retaining physical weight, pressure class, and component-type adjustments.
  const factorScale = hasBomFactorOverride && Number.isFinite(defaultMedianFactor) && defaultMedianFactor > 0
    ? bomFactorOverride / defaultMedianFactor
    : 1;
  const medianUnitRate = baseMedianUnitRate * factorScale;
  const p90UnitRate = baseP90UnitRate * factorScale;

  return {
    component: componentFactor.component,
    factor: hasBomFactorOverride ? bomFactorOverride : defaultDisplayFactor,
    componentFactor: componentFactor.factor,
    factorUom: componentFactor.uom,
    confidence: componentFactor.confidence,
    ...
    rawSteelOverrideApplied: hasBomRawOverride,
    factorOverrideApplied: hasBomFactorOverride,
...
function buildBomGroupItem(row, columns, sourceName, sourceKey) {
  ...
  const rawOverrideText = columns.rawOverride ? row[columns.rawOverride] : "";
  const factorOverrideText = columns.factorOverride ? row[columns.factorOverride] : "";
  ...
  return {
    ...
    rawSteelOverride: String(rawOverrideText || "").trim(),
    estimateFactorOverride: String(factorOverrideText || "").trim(),
    componentCost,
  };
}
                  ...
                  .map((item) => {
                    ...
                    return `
                      <tr class="${needsReview ? "review-row" : ""}">
                        ...
                        <td data-label="Raw Steel Rs/kg">${Number.isFinite(cost.rawSteel) ? formatCurrency(cost.rawSteel, 2) : "Review"}${
                          cost.rawSteelOverrideApplied ? '<span class="fallback-pill">BOM override</span>' : ""
                        }</td>
                        ...
                        <td data-label="Factor">${
                          ...
                          cost.factorOverrideApplied
                            ? '<span class="fallback-pill">BOM override</span>'
                            : ""
                        }${
  ...
  rows.forEach((row) => {
    const groupItem = buildBomGroupItem(
      row,
      {
        ...
        uom: uomColumn,
        spec: specColumn,
        rawOverride: rawColumn,
        factorOverride: factorColumn,
...
function getOverrideNotes(items = getReportItems(), useHtml = false) {
  ...
  bomGroupItems.forEach((item) => {
    const cost = item.componentCost || {};
    if (!cost.rawSteelOverrideApplied && !cost.factorOverrideApplied) return;

    notes.push(
      `Component ${item.item || "row"}: ${
        cost.rawSteelOverrideApplied
          ? `raw steel override Rs ${useHtml ? highlightOverride(formatNumber(cost.rawSteel, 2)) : formatNumber(cost.rawSteel, 2)} per kg; `
          : ""
      }${
        cost.factorOverrideApplied
          ? `median factor override ${useHtml ? highlightOverride(formatNumber(cost.factor, 2)) : formatNumber(cost.factor, 2)}; `
          : ""
      }applied from the uploaded BOM row.`
    );
  });
...
function buildEditableComponentSheet(groupName, groupItems) {
  const workbookRows = [
    ...
    [],
   [
     "Item", "Size", "Rating / Thk", "Material", "Material Category", "Raw Steel Rs/kg", "Qty", "UOM",
  ...
  groupItems.forEach((item, itemIndex) => {
    ...
    const detail = [
     item.item || "", item.size || "", item.thickness || "", item.material || "",
      formatCategoryHeading(item.materialCategory || "Unclassified"),
      Number.isFinite(cost.rawSteel) ? Number(cost.rawSteel) : "", hasQuantity ? quantity : "", item.uom || "",
    ...
    detail.forEach((value, column) => {
      ...
      const isEditable = [6, 8, 9, 11].includes(column) && value !== "";
      const isRawSteelAuditRate = column === 5 && typeof value === "number";
      setExcelCell(sheet, address, value, isEditable ? (column === 8 ? currencyEditStyle : editStyle) : isRawSteelAuditRate ? currencyFormulaStyle : standardStyle);
    });

    const unitAddress = `K${rowNumber}`;
    const normalAddress = `M${rowNumber}`;
    const p90Address = `N${rowNumber}`;
    setExcelCell(sheet, unitAddress, {
      t: hasUnitRate ? "n" : "s",
      f: `IF(AND(ISNUMBER(I${rowNumber}),ISNUMBER(J${rowNumber})),I${rowNumber}*J${rowNumber},"Review")`,
      v: Number.isFinite(unitRate) ? unitRate : "Review",
    }, hasUnitRate ? currencyFormulaStyle : reviewStyle);
    setExcelCell(sheet, normalAddress, {
      t: hasTotalRate ? "n" : "s",
      f: `IF(AND(ISNUMBER(G${rowNumber}),ISNUMBER(K${rowNumber})),G${rowNumber}*K${rowNumber},"Review")`,
      v: hasTotalRate ? Number(cost.medianTotal) : "Review",
    }, hasTotalRate ? currencyFormulaStyle : reviewStyle);
    setExcelCell(sheet, p90Address, {
      t: hasTotalRate ? "n" : "s",
      f: `IF(AND(ISNUMBER(M${rowNumber}),ISNUMBER(L${rowNumber})),M${rowNumber}*L${rowNumber},"Review")`,
      v: hasTotalRate ? Number(cost.p90Total) : "Review",
    }, hasTotalRate ? currencyFormulaStyle : reviewStyle);
    if (!hasUnitRate || !hasTotalRate) {
      setExcelCell(sheet, `O${rowNumber}`, "Review", reviewStyle);
    }
  });

  const totalRow = groupItems.length + 8;
  setExcelCell(sheet, `A${totalRow}`, "Group total", { font: { bold: true, color: { rgb: "0B365F" } } });
  setExcelCell(sheet, `M${totalRow}`, { t: "n", f: `SUM(M6:M${totalRow - 2})`, v: getComponentCostSummary(groupItems).normal }, currencyFormulaStyle);
  setExcelCell(sheet, `N${totalRow}`, { t: "n", f: `SUM(N6:N${totalRow - 2})`, v: getComponentCostSummary(groupItems).p90 }, currencyFormulaStyle);
  setExcelCell(sheet, `A${totalRow + 2}`, "Workbook note", { font: { bold: true, color: { rgb: "0B365F" } } });
  setExcelCell(sheet, `B${totalRow + 2}`, "Raw Steel Rs/kg is the audit rate used by the app. Base Rate Rs, Factor, Qty and P90 Ratio are editable. The app-calculated Unit Rs is represented as Base Rate x Factor so commercial changes flow into all totals.", standardStyle);
  sheet["!merges"] = [{ s: { r: 0, c: 0 }, e: { r: 0, c: 15 } }, { s: { r: 2, c: 0 }, e: { r: 2, c: 15 } }, { s: { r: totalRow + 1, c: 1 }, e: { r: totalRow + 1, c: 15 } }];
  sheet["!cols"] = [
   { wch: 26 }, { wch: 13 }, { wch: 17 }, { wch: 24 }, { wch: 25 }, { wch: 10 }, { wch: 10 },
   { wch: 15 }, { wch: 11 }, { wch: 14 }, { wch: 12 }, { wch: 18 }, { wch: 17 }, { wch: 13 }, { wch: 48 },
    { wch: 26 }, { wch: 13 }, { wch: 17 }, { wch: 24 }, { wch: 25 }, { wch: 17 }, { wch: 10 }, { wch: 10 },
    { wch: 15 }, { wch: 11 }, { wch: 14 }, { wch: 12 }, { wch: 18 }, { wch: 17 }, { wch: 13 }, { wch: 48 },
  ];
  ...
  sheet["!autofilter"] = { ref: `A5:P${Math.max(groupItems.length + 5, 5)}` };
...
function exportEditableBomExcelReport() {
  ...
  dashboardGroups.forEach((group, index) => {
    ...
    setExcelCell(executiveSheet, `C${row}`, { t: "n", f: `${quotedSheetName}!M${group.totalRow}`, v: group.summary.normal }, currencyStyle);
    setExcelCell(executiveSheet, `D${row}`, { t: "n", f: `${quotedSheetName}!N${group.totalRow}`, v: group.summary.p90 }, currencyStyle);
                ...
                .map((item) => {
                  ...
                  return `
                    <tr class="${needsReview ? "pending-row" : ""}">
                      ...
                      <td>${Number.isFinite(cost.rawSteel) ? formatCurrency(cost.rawSteel, 2) : "Review"}${
                        cost.rawSteelOverrideApplied ? ' <small class="fallback">BOM override</small>' : ""
                      }</td>
                     ...
                     <td>${Number.isFinite(cost.factor) ? formatNumber(cost.factor, 2) : "-"}${
                        cost.factorOverrideApplied
                          ? ' <small class="fallback">BOM override</small>'
                          : ""
                      }${
                       cost.isGenericFallback
